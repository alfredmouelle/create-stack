...
Reference Next.js (App Router) app — the starting point to fork for a new
project. Carries the personal baseline (strict Biome via the repo root config,
`~/*` alias, typed `env.ts`) and nothing else: tools are added per-project with
the **add-capability** skill, foundations (trpc, better-auth, drizzle) with
**bootstrap**.
